...
| ID | Sev | Issue | Fix sketch |
|---|---|---|---|
| **PL-1** | P1 | In-progress milestone won't pull earlier (§4.1). If predecessor finishes ahead of plan, in-progress milestone stays at its old start instead of advancing. | Document explicitly in drawer ("in-progress dates are protected from forward-pull"); offer a per-row "allow pull" toggle. Not auto-changeable — keep the protection. |
| **PL-2** | P0 | Task→milestone push is **one-hop only**. If a task pushes milestone m6 in tasks-grid, the engine does NOT then run `previewCascade` to see if m6 pushes m7 (which depends on m6). M20.3's claim of "fully transitive" is currently false. | After `previewTaskToMilestonePush`, run `previewCascade` with each proposed shift as an edit; merge the resulting milestone shifts into the drawer's milestone section. Re-test 3-hop scenarios. |
| **PL-3** | P1 | `daysShifted` everywhere uses `daysBetween` (calendar days) but constraints use working days. A 7-calendar-day shift over a weekend = 5 working days. PM sees inconsistent numbers. | Add `workingDaysBetween(a, b, workingDays, holidays)` helper; switch shift displays to working days while keeping calendar days in tooltips / Gantt rendering. |
| **PL-4** | P1 | `previewTaskToMilestonePush` proposes milestone date = task dueDate exactly. Milestones usually represent a gate review *after* their final task, so the proposed date is too aggressive by some buffer. | Add a configurable "milestone gate buffer" (default 1 working day) added to `proposedNewDate`. Surface in drawer ("m6 pushed to 2026-05-22, 1 day after last task t8"). |
...
| **PL-11** | **P0** | **Cascade silently "auto-fixes" pre-existing violations downstream of any edit, even a no-op edit.** Discovered while building the M20.4 test matrix: calling `previewTaskCascade` with `newDueDate === currentDueDate` still produces shifts on downstream tasks that were pre-existing violations. The drawer attributes these to "this edit" via `affected[]`, which is misleading. A "phantom save" (re-saving the same date) can quietly re-date tasks. | Engine takes a `respectPreExisting?: boolean` opt (default `true` for the drawer). When `true`: compute the baseline pass on input data, then run the cascade, then suppress any shift whose pre-cascade violation already existed in baseline. The drawer M20.1 diff handles violations; this fix handles dates. |
